...
// Deletes a single image from Cloudinary via the Admin API (same Basic
// Auth pattern as listImages above). Used when someone removes a gallery
// photo or a leadership Post from their workspace.
async function deleteImage(publicId) {
  const auth = Buffer.from(`${process.env.CLOUDINARY_API_KEY}:${process.env.CLOUDINARY_API_SECRET}`).toString('base64');
  const url = `https://api.cloudinary.com/v1_1/${process.env.CLOUDINARY_CLOUD_NAME}/resources/image/upload?public_ids[]=${encodeURIComponent(publicId)}`;
  const resp = await fetch(url, { method: 'DELETE', headers: { 'Authorization': `Basic ${auth}` } });
  if (!resp.ok) {
    const errBody = await resp.json().catch(() => ({}));
    throw new Error(errBody?.error?.message || `Cloudinary delete failed (${resp.status})`);
  }
  return resp.json();
}

module.exports = { isConfigured, signParams, listImages, deleteImage };
